...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		if (sharedInstance != null && args == null) {
			...
			bean = getObjectForBeanInstance(sharedInstance, name, beanName, null); // 如果是FactoryBean，从这里得到目标bean，即FactoryBean.getObject()实例
		}
			...
			try {
				...
				if (mbd.isSingleton()) {// 单例
					sharedInstance = getSingleton(beanName, () -> {
						try {
							return createBean(beanName, mbd, args);// 创建单例 bean 的入口
						}
	...
	public Class<?> getType(String name, boolean allowFactoryBeanInit) throws NoSuchBeanDefinitionException {
		...
		// Check manually registered singletons.
		Object beanInstance = getSingleton(beanName, false);// 从单例池获取，限制只能从第一、二级缓存获取
		if (beanInstance != null && beanInstance.getClass() != NullBean.class) {
	...
	protected Object getObjectForBeanInstance(
		...
		if (BeanFactoryUtils.isFactoryDereference(name)) {// 如果包含了&前缀，则返回FactoryBean本身
		...
		if (mbd != null) {
			...
			object = getCachedObjectForFactoryBean(beanName);// 从缓存中获取FactoryBean的目标Bean
		}
		if (object == null) {
			...
			object = getObjectFromFactoryBean(factory, beanName, !synthetic);// 生成FactoryBean的目标Bean并放入缓存
		}
		return object;
	}
