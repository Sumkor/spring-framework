...
public class MyBeanAutowiredTest {
	...
	public static void main(String[] args) {
		 ...
		 * 为什么需要三级缓存？
		 *
		 * 第一级缓存是 singletonObjects，即单例池，存储完成所有初始化的 bean.
		 * 第二级缓存是 earlySingletonObjects，为早期曝光对象，存储未完成初始化的早期 bean.
		 * 第三级缓存是 singletonFactories，为早期曝光对象工厂，存储处理 bean 的函数.
		 *
		 * 如果只是解决普遍 bean 的循环依赖，两级缓存就足够了。
		 * 提前曝光时将早期 bean 存储在第二级缓存，若其他 bean 依赖了该 bean，从第二级缓存获取即可。待该 bean 执行完生命周期所有初始化后再存入第一级缓存。
		 *
		 * 但是，如果是解决动态代理的 bean 的循环依赖，则需要三级缓存。
		 * 因为提前曝光时，存储在第三级缓存的 singletonFactory 对象是一个 lambda 函数，当其他 bean 依赖当前 bean 时，才从第三级缓存中获取并执行该 lambda 函数生成代理对象。
		 * 如果只有二级缓存，在提前曝光时，不管有没有被其他 bean 依赖，都需要提前生成代理对象存入二级缓存，这就打乱了 bean 生命周期的正常流程。
		 *
		 * 此外，三级缓存提供了扩展点。
